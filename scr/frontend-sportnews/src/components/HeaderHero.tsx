...
const menu = [
  { label: "Chính trị", link: "/chinh-tri" },
  { label: "Kinh doanh", link: "/kinh-doanh" },
  { label: "Giải trí", link: "/giai-tri" },
  { label: "Công nghệ", link: "/cong-nghe" },
  { label: "Thể thao", link: "/the-thao" },
  { label: "Sức khỏe", link: "/suc-khoe" },
  { label: "Liên hệ với chúng tôi", link: "/lien-he" },
];
...
function HeaderHero() {
  ...
  const onSearch = () => {
    const keyword = queryString.stringify({ q: search });
  ...
  return (
    <header className="container">
        ...
        <div className="header-top-right">
          <div className="search">
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  onSearch(); // Gọi hàm xử lý khi nhấn Enter
                }
              }}
              name="text"
              type="text"
      ...
      <div className="header-bottom">
        {menu.map((item, index) => (
          <Link className="header-bottom-link" key={index} to={item.link}>
            {item.label}
          </Link>
        ))}
        {token ? (
          <div style={{ display: "flex", gap: "6px" }}>
            ...
            <div onClick={handleLogout} className="header-bottom-link">
              {" "}
              Đăng Xuất
            </div>
          </div>
          ...
          <div>
            <Link to={"/dang-nhap"} className="header-bottom-link">
              {" "}
              Đăng nhập
            </Link>{" "}
            /
            <Link to={"/dang-ky"} className="header-bottom-link">
              {" "}
              Đăng ký
            </Link>
          </div>
